...
# API Key Connections
def Api_Connect():
    api_id = 'AIzaSyCrAx50BFJDSP3RmvsU-CJMrzrUp2lWVCk'
...
def get_comment_details(video_ids):
  ...
  try:
    for video_id in video_ids:
      resquest=youtube.commentThreads().list(
        part="snippet",
        videoId=video_id,
        maxResults=50
      )
...
# Upload to mongoDB
client=pymongo.MongoClient("mongodb+srv://raghulkumar3:Raghul@cluster0.s06bw.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
db=client["youtube"]
...
def channel_details(channel_id):
    ...
    return "Uploaded completed to MongoDB successfully"


#Table creation for channel, playlists,vidoe,comment
def channels_table():
    mydb =psycopg2.connect(host="localhost",
                            user="postgres",
                            password="Raghulkumar8076",
...
with st.sidebar:
    st.title(":green[YOUTUBE DATA HAVERSTINGAND WAREHOUSING]")
    st.header("Skill Development")
    st.caption("Python Scripting")
    st.caption("Data Collections")
    st.caption("MongoDB")
    st.caption("API Integration")
    st.caption("Data Managemant using MongoDB and SQL")
...
if st.button("collect and store data"):
    ...
    if channel_id in ch_ids:
        st.success("The given channel details of the channel id is already exists")
...
if question=="1.What are the names of all the videos and their corresponding channels?":
    ...
    query3='''select views as views,channel_name aschannelname,title as videotitle from videos
